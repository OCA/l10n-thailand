...
class RdTaxReportView(models.TransientModel):
    _name = "rd.tax.report.view"
    _description = "Tax RD Report View"
    _order = "id"
...
class RdTaxReport(models.TransientModel):
    _name = "report.rd.tax.report"
    _description = "Report Tax RD Report"
    ...
    results = fields.Many2many(
        comodel_name="rd.tax.report.view",
    ...
    def _compute_results(self):
        ...
        ReportLine = self.env["rd.tax.report.view"]
        self.results = False
    ...
    def print_report(self, report_type="qweb-pdf"):
        self.ensure_one()
        action = self.env.ref("l10n_th_account_tax_report_rd.action_rd_tax_report_pdf")
        return action.report_action(self, config=False)

    def _get_period_be(self, date_range_id):
        month = "-"
        year = "-"
        date_start = (date_range_id.date_start + relativedelta(years=543)).strftime(
            "%m-%Y"
        )
        date_end = (date_range_id.date_end + relativedelta(years=543)).strftime("%m-%Y")
