...
class ResCurrencyRateProviderBOT(models.Model):
    ...
    def _get_supported_currencies(self):
        ...
        return [
            'USD', 'GBP', 'EUR', 'JPY', 'HKD', 'MYR', 'SGD', 'BND', 'PHP', 'IDR',
            'INR', 'CHF', 'AUD', 'NZD', 'PKR', 'CAD', 'SEK', 'DKK', 'NOK', 'CNY',
            'MXN', 'ZAR', 'MMK', 'KRW', 'TWD', 'KWD', 'SAR', 'AED', 'BDT', 'CZK',
            'KHR', 'KES', 'LAK', 'RUB', 'VND', 'EGP', 'PLN', 'LKR', 'IQD', 'BHD',
            'OMR', 'JOD', 'QAR', 'MVR', 'NPR', 'PGK', 'ILS', 'HUF'
        ]
    ...
    def _obtain_rates(self, base_currency, currencies, date_from, date_to):
        ...
        if self.service == "BOT":
            ...
            if not bot_client_id:
                raise UserError(_(
                    'No bot.or.th credentials specified!'
                ))
            ...
            for data_detail in data_details:
                period = fields.Date.from_string(data_detail['period']) \
                    .strftime(DEFAULT_SERVER_DATE_FORMAT) \
                    if data_detail['period'] else False
