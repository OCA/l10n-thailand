...
class WizardCurrencyRevaluation(models.TransientModel):
    ...
    def _prepare_base_move(self, amount, sums, form, currency_id):
        base_move = {
            ...
            "ref": sums.get("move_name"),
            "revaluation_to_reverse": True,
        }
        return base_move

    def _prepare_base_move_line(
        self,
        amount,
        debit_account_id,
        credit_account_id,
        ...
        base_move_line = {
            ...
            "amount_currency": 0.0,
            "date": form.revaluation_date,
            "gl_foreign_balance": sums.get("foreign_balance", 0.0),
            "gl_balance": sums.get("balance", 0.0),
            "gl_revaluated_balance": sums.get("revaluated_balance", 0.0),
            "gl_currency_rate": sums.get("currency_rate", 0.0),
        }
        return base_move_line
    ...
    def _create_move_and_lines(
        ...
        base_move = self._prepare_base_move(amount, sums, form, currency_id)
        base_line = self._prepare_base_move_line(
            amount,
            debit_account_id,
            credit_account_id,
            sums,
        ...
        revaluation_origin_line_ids = sums.get("id", False)
    ...
    def _validate_company_revaluation_configuration(self, company):
        return (
            company.revaluation_loss_account_id and company.revaluation_gain_account_id
        )
        ...
        for account_id, by_account in revaluations.items():
            ...
            for partner_id, by_partner in by_account.items():
                for currency_id, lines in by_partner.items():
                    ...
                    currency_lines = lines
                    for line in currency_lines:
                        diff_balances = self._compute_unrealized_currency_gl(
                            currency, line
                        )
                        line.update(diff_balances)
                    revaluations[account_id][partner_id][currency_id] = currency_lines

        # Create entries only after all computation have been done
        created_ids = []
        move_reversed = move_object.search([("revaluation_reversed", "=", True)])
        for account_id, by_account in revaluations.items():
            account = Account.browse(account_id)
            ...
            for partner_id, by_partner in by_account.items():
                for currency_id, lines in by_partner.items():
                    ...
                    for line in lines:
                        adj_balance = line.get("unrealized_gain_loss", 0.0)
                        if currency.is_zero(adj_balance):
                            continue
                        rate = line.get("currency_rate", 0.0)
                        # Check condition to skip revalue
                        # 1. currency rate must be not the same between
                        # the Revaluation Date and the latest currency rate
                        # of the document.
                        # 2. move is not reversed.
                        ml_origin = ml_object.browse(line["id"])
                        move_reversed = move_reversed.filtered(
                            lambda l: l.reversed_entry_id
                            == ml_origin.revaluation_created_line_id.move_id
                        )
                        if (
                            float_compare(
                                rate,
