...
class TestVendorTaxInvoice(SingleTransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.invoice_vat = cls.env['account.invoice'].create(invoice_dict)
        ...
        cls.invoice_undue_vat = cls.env['account.invoice'].create(invoice_dict)
    ...
    def test_supplier_invoice_vat(self):
        ...
        with self.assertRaises(UserError) as e:
            self.invoice_vat.action_invoice_open()
        self.assertEqual(e.exception.name,
                         'Please fill in tax invoice and tax date')
        tax_invoice = 'SINV-10001'
        tax_date = fields.Date.today()
        self.invoice_vat.tax_line_ids.write({
            'tax_invoice_manual': tax_invoice,
            'tax_date_manual': tax_date, })
        self.invoice_vat.action_invoice_open()
        # Check that tax_invoice and date is stamped properly
        for tax_line in self.invoice_vat.tax_line_ids:
            ml = self.env['account.move.line'].search([
                ('invoice_tax_line_id', '=', tax_line.id)])
            self.assertEqual(len(ml), 1)
            self.assertEqual(ml.tax_invoice, tax_invoice)
            self.assertEqual(ml.tax_date, str(tax_date))

    def test_supplier_invoice_undue_vat_full_payment(self):
        """ Supplier Invoice with Undue VAT,
        user fill Tax Invoice/Date on Payment """
        # Do not allow user to fill in Tax Invoice/Date
        tax_invoice = 'SINV-10001'
        tax_date = fields.Date.today()
        self.invoice_undue_vat.tax_line_ids.write({
            'tax_invoice_manual': tax_invoice,
            'tax_date_manual': tax_date, })
        with self.assertRaises(UserError) as e:
            self.invoice_undue_vat.action_invoice_open()
        self.assertEqual(e.exception.name,
                         'Please do not fill in tax invoice for undue tax')
        # Clear tax invoice/date from account.invoice.tax, and confirm
        self.invoice_undue_vat.tax_line_ids.write({
        ...
        payment = self.env['account.payment'].create({
            ...
            'invoice_ids': [(4, self.invoice_undue_vat.id, None)],
        })
        payment.post()
        ...
        payment.clear_tax_cash_basis()
        # Check that tax_invoice and date is stamped properly on tax line
