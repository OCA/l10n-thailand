# Copyright 2021 Ecosoft Co., Ltd. (http://ecosoft.co.th)
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).

from odoo import Command, api, fields, models


class AccountMove(models.Model):
    _inherit = "account.move"

    guarantee_ids = fields.Many2many(
        comodel_name="purchase.guarantee",
        relation="account_move_guarantee_rel",
        column1="move_id",
        column2="guarantee_id",
        string="Guarantee",
        ondelete="restrict",
    )
    return_guarantee_ids = fields.Many2many(
        comodel_name="purchase.guarantee",
        relation="account_move_return_guarantee_rel",
        column1="move_id",
        column2="guarantee_id",
        string="Return Guarantee",
        ondelete="restrict",
    )

    def _prepare_guarantee_move_line(self, guarantee):
        self.ensure_one()
        return {
            "name": guarantee.guarantee_method_id.name,
            "account_id": guarantee.guarantee_method_id.account_id.id,
            "quantity": 1,
            "price_unit": guarantee.amount,
            "analytic_distribution": guarantee.analytic_distribution,
            "analytic_tag_ids": [Command.set(guarantee.analytic_tag_ids.ids)],
        }

    @api.onchange("guarantee_ids")
    def _onchange_guarantee_ids(self):
        # Clear invoice lines
        self.invoice_line_ids = [Command.clear()]

        move_line_model = self.env["account.move.line"]
        # New invoice lines
        for guarantee in self.guarantee_ids:
            self.invoice_line_ids += move_line_model.new(
                self._prepare_guarantee_move_line(guarantee)
            )

    @api.onchange("return_guarantee_ids")
    def _onchange_return_guarantee_ids(self):
        # Clear invoice lines
        self.invoice_line_ids = [Command.clear()]

        move_line_model = self.env["account.move.line"]
        # New invoice lines
        for line in self.return_guarantee_ids.mapped("invoice_ids.invoice_line_ids"):
            self.invoice_line_ids += move_line_model.new(
                {field: line[field] for field in list(line._fields.keys())}
            )

    @api.onchange("partner_id")
    def _onchange_partner_id(self):
        res = super()._onchange_partner_id()
        # Clear guarantee fields
        self.write({"guarantee_ids": False, "return_guarantee_ids": False})
        return res
