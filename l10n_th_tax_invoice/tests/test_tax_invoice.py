...
class TestTaxInvoice(SingleTransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.supplier_invoice_vat = \
            cls.env['account.move'].create(invoice_dict)
    ...
    def test_supplier_invoice_undue_vat_payment(self):
        ...
        payment.tax_invoice_ids.write({'tax_invoice_number': tax_invoice,
                                       'tax_invoice_date': tax_date})
        payment.clear_tax_cash_basis()
        # Cash basis journal is now posted
        self.assertEquals(payment.tax_invoice_ids.mapped('move_id').state,
                          'posted')
