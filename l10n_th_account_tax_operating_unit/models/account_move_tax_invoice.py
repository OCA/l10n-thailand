...
class AccountMoveTaxInvoice(models.Model):
    ...
    @api.depends("payment_id", "payment_id.operating_unit_id", "move_id", "move_id.operating_unit_id", "move_line_id", "move_line_id.operating_unit_id")
    def _compute_operating_unit_id(self):
        default_ou = self.env["res.users"].operating_unit_default_get(self.env.uid)
        for rec in self:
            ou = default_ou
            if rec.move_line_id and rec.move_line_id.operating_unit_id:
                ou = rec.move_line_id.operating_unit_id
