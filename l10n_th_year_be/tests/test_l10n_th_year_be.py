...
from odoo.addons.web.models.ir_http import Http as WebHttp
...
class TestL10nThYearBe(TransactionCase):
    ...
    def test_session_info_includes_year_be_payload(self):
        ...
        with patch.object(WebHttp, "session_info", return_value={"base_key": True}):
            session_info = self.env["ir.http"].session_info()

        self.assertIn("l10n_th_date_year_format", session_info)
        self.assertIn("l10n_th_date_auto_convert_be_input", session_info)
        self.assertTrue(session_info["base_key"])
